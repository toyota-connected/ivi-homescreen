...
The same keys appear in
[`docs/config-examples/reference.toml`](../../docs/config-examples/reference.toml)
and in the generated table in
[`shell/configuration/README.md`](../configuration/README.md):
`scripts/gen_config_reference.py` scrapes `shell/osgi/osgi_config.cc` alongside
the `[[view]]` parser, so a key added there fails the build of the reference
until it is documented. The table above is the one to read for bounds; the
generated one exists so no key can ship undocumented.
