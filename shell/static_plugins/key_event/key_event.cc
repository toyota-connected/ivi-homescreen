...
DelegateHandleKey::DelegateHandleKey(void *data, DelegateHandleKey::HandleKeyHook hook,
        FlutterKeyEventType type, uint32_t xkb_scancode, xkb_keysym_t keysym) :
      m_data(data), m_hook(hook), m_type(type),
      m_xkb_scancode(xkb_scancode), m_keysym(keysym) {}
...
KeyEvent::FL_KEY_EV_RET_T KeyEvent::ParseReply(const uint8_t* reply, size_t reply_size) {
  auto ret = FL_KEY_EV_RET_FAILED;

  if (reply_size == 0) {
    FML_DLOG(ERROR) << "KeyEvent: BinaryReply: reply_size is 0";
    return ret;
  }
  ...
  if (decoded == nullptr) {
    /* could not decode */
    FML_DLOG(ERROR) << "KeyEvent: BinaryReply: could not decoded";
    return ret;
  }

  auto handled = decoded->FindMember(kHandled);
  if (handled == decoded->MemberEnd()) {
    FML_DLOG(ERROR) << "KeyEvent: BinaryReply: could not found key \"" << kHandled << "\"";

    // check the reply contents
    for (auto& m : decoded->GetObject()) {
      FML_DLOG(ERROR) << "key: " << m.name.GetString() << " type: " << m.value.GetType();
    }
  ...
  if (!(handled->value.IsBool())) {
    FML_DLOG(ERROR) << "KeyEvent: BinaryReply: key \"" << kHandled << "\" is not bool. unexpected.";
    return ret;
  }

  ret = handled->value.GetBool() ? FL_KEY_EV_RET_HANDLED : FL_KEY_EV_RET_IGNORED;
...
void KeyEvent::SendKeyEvent(bool released,
  ...
  // flutter-engine will return the result which if a key event is handled or ignored.
  // If a key is handled, the key event must not be propagated to any other plugins e.g. TextInput.
  // If a key is ignored, the key event will be propagated to other plugins e.g. TextInput.
  // So, the reply from flutter-engine needs to be received.
  channel_->Send(event, [delegate = std::move(delegate)](const uint8_t* reply, size_t reply_size) {
    if (delegate == nullptr) {
      // delegate is not set, parsing the keyevent result is meaningless, so skip
      return;
    }
    ...
    case FL_KEY_EV_RET_IGNORED:
      // If parsing is failed, some issues might happen on the side of flutter app.
      // However, here, what issue happened cannot be got know.
      // So, the failure of parsing is determined as same as ignored.
...
void KeyEvent::keyboard_handle_key(void* data,
  ...
  d->SendKeyEvent(eventType == kFlutterKeyEventTypeUp, keysym, xkb_scancode, 0, std::move(delegate));
}
