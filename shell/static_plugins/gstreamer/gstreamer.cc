...
void handoff_handler(GstElement* fakesink,
  ...
  if (gst_video_frame_map(&frame, &data->info, buffer, GST_MAP_READ)) {
    data->engine->GetEglWindow()->MakeTextureCurrent();
    ...
    draw_core(data->shader);
    data->engine->GetEglWindow()->ClearCurrent();
    ...
    case GST_MESSAGE_BUFFERING: {
      ...
      if (percent == 100) {
        ...
        if (data->is_buffering) {
          ...
          ss_event_name << kChannelGstreamerEventPrefix << data->texture;
...
void Gstreamer::OnInitialize(const FlutterPlatformMessage* message,
                             void* userdata) {
  ...
  PlatformChannel::GetInstance()->RegisterCallback(kChannelGstreamerPause,
                                                   OnPause);
  PlatformChannel::GetInstance()->RegisterCallback(
      kChannelGstreamerSetMixWithOthers, &Gstreamer::OnSetMixWithOthers);
...
void Gstreamer::OnCreate(const FlutterPlatformMessage* message,
                         void* userdata) {
  PrintMessageAsHex(message);
  auto engine = reinterpret_cast<Engine*>(userdata);
  auto& codec = flutter::StandardMessageCodec::GetInstance();
  ...
  engine->GetEglWindow()->MakeTextureCurrent();
  ...
  glFinish();
  engine->GetEglWindow()->ClearCurrent();
...
void Gstreamer::OnDispose(const FlutterPlatformMessage* message,
                          void* userdata) {
  PrintMessageAsHex(message);
  auto engine = reinterpret_cast<Engine*>(userdata);
  auto& codec = flutter::StandardMessageCodec::GetInstance();
...
void Gstreamer::OnSetLooping(const FlutterPlatformMessage* message,
                             void* userdata) {
  PrintMessageAsHex(message);
  auto engine = reinterpret_cast<Engine*>(userdata);
  auto& codec = flutter::StandardMessageCodec::GetInstance();
...
void Gstreamer::OnSetPlaybackSpeed(const FlutterPlatformMessage* message,
                                   void* userdata) {
  PrintMessageAsHex(message);
  auto engine = reinterpret_cast<Engine*>(userdata);
  auto& codec = flutter::StandardMessageCodec::GetInstance();
...
void Gstreamer::OnPlay(const FlutterPlatformMessage* message, void* userdata) {
  PrintMessageAsHex(message);
  auto engine = reinterpret_cast<Engine*>(userdata);
  auto& codec = flutter::StandardMessageCodec::GetInstance();
  auto obj = codec.DecodeMessage(message->message, message->message_size);
...
void Gstreamer::OnPosition(const FlutterPlatformMessage* message,
                           void* userdata) {
  PrintMessageAsHex(message);
  auto engine = reinterpret_cast<Engine*>(userdata);
  auto& codec = flutter::StandardMessageCodec::GetInstance();
...
void Gstreamer::OnSeekTo(const FlutterPlatformMessage* message,
                         void* userdata) {
  PrintMessageAsHex(message);
  auto engine = reinterpret_cast<Engine*>(userdata);
  auto& codec = flutter::StandardMessageCodec::GetInstance();
...
void Gstreamer::OnPause(const FlutterPlatformMessage* message, void* userdata) {
  PrintMessageAsHex(message);
  auto engine = reinterpret_cast<Engine*>(userdata);
  auto& codec = flutter::StandardMessageCodec::GetInstance();
  auto obj = codec.DecodeMessage(message->message, message->message_size);
...
void Gstreamer::OnSetMixWithOthers(const FlutterPlatformMessage* message,
                                   void* userdata) {
  auto engine = reinterpret_cast<Engine*>(userdata);
  auto& codec = flutter::StandardMessageCodec::GetInstance();
  auto obj = codec.DecodeMessage(message->message, message->message_size);
