...
// by asking the engine. Two things need that answer:
//
...
//
//   - drm_kms_vulkan picks its presentation path on it. The embedder cannot
//     turn a Vulkan backing store into an Impeller render target -- embedder.cc
//     logs "Unimplemented" for kFlutterBackingStoreTypeVulkan -- so under
//     Impeller that backend must offer no compositor and present through the
//     root surface instead.
//
...
// The renderer the engine was actually started with, recorded once the view has
// assembled the final switch list and readable from anywhere afterwards.
//
// A backend cannot answer this from its own construction arguments: it is built
// before FlutterView::Initialize assembles command_line_argv, and that assembly
// can add switches of its own (headless-egl turns Impeller on there). Callbacks
// like Backend::GetCompositorConfig take no arguments and run later still, so
// they need somewhere to look it up rather than something to be handed.
namespace ihs::engine_switches {

inline std::atomic<bool>& ImpellerActiveStorage() {
  static std::atomic<bool> active{false};
  return active;
}

// Called once per process by the view, after the switch list is final.
inline void SetImpellerActive(const bool active) {
  ImpellerActiveStorage().store(active, std::memory_order_release);
}

// False until the view has recorded the answer, which is the correct reading
// during startup: nothing has asked the engine to run Impeller yet.
[[nodiscard]] inline bool ImpellerActive() {
  return ImpellerActiveStorage().load(std::memory_order_acquire);
}
