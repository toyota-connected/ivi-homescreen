...
3. **Per-thread RT priority via `thread_priority_setter`** (`5ae8ae50`). Flutter's rasterizer gets `SCHED_FIFO` prio 10; UI thread gets `SCHED_FIFO` prio 5; background pool gets `SCHED_BATCH`. Gated by `IVI_DRM_RT` because a runaway RT thread is unrecoverable without a hard reset. Eliminating raster-thread preemption jitter is the single biggest contributor at high refresh rates.
