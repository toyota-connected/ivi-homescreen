...
Egl_headless::~Egl_headless() {
  eglTerminate(m_dpy);
  eglReleaseThread();
}

bool Egl_headless::MakeCurrent() {
...
bool Egl_headless::ClearCurrent() {
...
bool Egl_headless::MakeResourceCurrent() {
...
bool Egl_headless::MakeTextureCurrent() {
...
bool Egl_headless::HasEGLExtension(const char* extensions, const char* name) {
...
std::array<EGLint, 4> Egl_headless::RectToInts(const FlutterRect rect) const {
  EGLint height;
...
void Egl_headless::print_extension_list(EGLDisplay dpy) {
...
void Egl_headless::ReportGlesAttributes(EGLConfig* configs, EGLint count) {
  std::stringstream ss;
...
void Egl_headless::sDebugCallback(EGLenum error,
                         const char* command,
                         EGLint messageType,
...
void Egl_headless::EGL_KHR_debug_init(const char* extensions) {
...
EGLSurface Egl_headless::create_egl_surface(void* native_window,
