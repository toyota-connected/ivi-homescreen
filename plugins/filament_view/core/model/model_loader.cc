...
void ModelLoader::populateScene(::filament::gltfio::FilamentAsset* asset) {
  ...
  while (count) {
    auto ready_entity = new utils::Entity[count];
    asset->popRenderables(ready_entity, count);
    for (int i = 0; i < count; i++) {
      auto ri = rcm.getInstance(ready_entity[i]);
      rcm.setScreenSpaceContactShadows(ri, true);
    }
    modelViewer_->getFilamentScene()->addEntities(ready_entity, count);
    delete[] ready_entity;
  ...
  auto lightTreeItem = [this, engine_, scene](utils::Entity entity) {
    auto& lm = engine_->getLightManager();
  ...
  applySettings(engine_, settings_.view, view);
  ...
  if (!isRemoteMode()) {
    ...
    if (animationCount > 0) {
      size_t selectedAnimation = currentAnimation_;
      // Disable
