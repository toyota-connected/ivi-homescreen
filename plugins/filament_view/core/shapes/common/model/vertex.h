...
class Vertex {
  ...
  var Box.centerPosition : Position get() =
      ...
      var Box.halfExtentSize
      : Size
        get() = halfExtent
                    .toSize()
                        set(value){setHalfExtent(value.x, value.y, value.z)}

                var Box.size get() =
          halfExtentSize * 2.0f set(value){halfExtentSize = value / 2.0f}

                           fun FloatArray.toPosition() =
              this.let{(x, y, z) -> Position(x, y, z)} fun FloatArray.toSize() =
                  this.let{(x, y, z)->Size(x, y, z)}

                  fun Mat4.toDoubleArray()
      : DoubleArray = toFloatArray().map{it.toDouble()}.toDoubleArray()
