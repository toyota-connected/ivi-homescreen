...
6. [Optional Features](#6-optional-features)
   ...
   - 6.2. [Crash Handler](#62-crash-handler) — `shell/crash_handler/crash_handler.{h,cc}`
...
Other loose files under `shell/` covered by this section: `crash_handler/crash_handler.{h,cc}`
...
[`shell/crash_handler/crash_handler.{h,cc}`](../../shell/crash_handler/crash_handler.h) provides Sentry-native crash handling: it uploads a crash report for triage. It is configured via the `SENTRY_DSN` environment variable
